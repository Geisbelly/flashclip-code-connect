  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        scrolled
          ? "bg-tech-blue/80 backdrop-blur-md shadow-lg"
          : "bg-transparent"
      }`}
    >
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center gap-4">
            <Button
              asChild
              variant="outline"
              className="hidden sm:flex border-tech-neon text-tech-neon hover:bg-tech-neon hover:text-tech-blue"
            >
      ...
      {isOpen && (
        <div className="absolute top-16 left-0 right-0 p-4 bg-tech-blue-light/95 backdrop-blur-md shadow-lg">
          <div className="flex flex-col space-y-2">
            <MobileNavLink to="/" icon={<div>{"{}"}</div>}>
              Home
            </MobileNavLink>
            <MobileNavLink to="/sobre" icon={<Star size={18} />}>
              Sobre
            </MobileNavLink>
            <MobileNavLink to="/cursos" icon={<Video size={18} />}>
              Cursos
            </MobileNavLink>
            <MobileNavLink to="/programacao" icon={<Calendar size={18} />}>
              Programação
            </MobileNavLink>
            <MobileNavLink to="/instrutores" icon={<User size={18} />}>
              Instrutores
            </MobileNavLink>
            <MobileNavLink to="/patrocinadores" icon={<div>$</div>}>
              Patrocinadores
            </MobileNavLink>
            <MobileNavLink to="/faq" icon={<div>?</div>}>
              FAQ
            </MobileNavLink>
            <MobileNavLink to="/contato" icon={<div>@</div>}>
              Contato
            </MobileNavLink>
            <Button
              asChild
              variant="outline"
              className="w-full mt-4 border-tech-neon text-tech-neon hover:bg-tech-neon hover:text-tech-blue"
            >
              <Link to="/inscricao">Inscreva-se</Link>
            </Button>
          </div>
