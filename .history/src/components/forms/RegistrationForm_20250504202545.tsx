...
const RegistrationForm = () => {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    formData.append("entry.123456789", values.fullName);      // Nome
    formData.append("entry.987654321", values.email);         // Email
    formData.append("entry.112233445", values.phone);         // Telefone
    formData.append("entry.556677889", values.institution);   // Instituição
    formData.append("entry.998877665", values.course);        // Curso
    formData.append("entry.112233448", values.miniCourse);    // Minicurso

    if (values.course === "outro") {
      formData.append("entry.667788990", "__other_option__");
      formData.append("entry.667788990_other_option_response", values.customCourse || "");
    }

    if (values.institution === "outro") {
      formData.append("entry.556677999", "__other_option__");
      formData.append("entry.556677999_other_option_response", values.customInstitution || "");
    }

    formData.append("entry.999999999", values.acceptTerms ? "Sim" : "Não"); // Termos
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-white">Telefone (Formato: 63 912345678) *</FormLabel>
              <FormControl>
                <Input 
            ...
            <FormItem>
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent className="bg-tech-blue-light border-white/10">
                  ...
                  <SelectItem value="uft">UNITINS</SelectItem>
                  <SelectItem value="uft">UNOPA</SelectItem>
                  <SelectItem value="uft">Catolica</SelectItem>
                  <SelectItem value="uft">Ulbrinha</SelectItem>
                  <SelectItem value="uft">CEM de Taquaralto</SelectItem>
                  <SelectItem value="outro">Outro</SelectItem>
                </SelectContent>
              </Select>
              <FormMessage />
