
import { FAQItem } from "@/components/common/FAQAccordion";

export const faqData: FAQItem[] = [
  {
    id: "faq1",
    question: "Como me inscrever para os cursos?",
    answer: "Para se inscrever, basta acessar a página de inscrição no site, preencher o formulário com seus dados pessoais e escolher o curso desejado. Após a confirmação, você receberá um email com mais informações."
  },
  {
    id: "faq2",
    question: "Posso me inscrever em mais de um curso?",
    answer: "Infelizmente, devido à sobreposição de horários, cada participante só pode se inscrever em um curso por edição do evento. Recomendamos escolher o que mais se alinha aos seus objetivos de aprendizado."
  },
  {
    id: "faq3",
    question: "Receberei certificado?",
    answer: "Sim, todos os participantes que completarem pelo menos 75% da carga horária do curso receberão um certificado digital de participação, que poderá ser baixado diretamente do site após o evento."
  },
  {
    id: "faq4",
    question: "Preciso levar meu próprio computador?",
    answer: "Para os cursos presenciais, a instituição disponibiliza computadores equipados com todos os softwares necessários. Para os cursos virtuais, você precisará de um computador com acesso à internet e os requisitos mínimos informados por email após a inscrição."
  },
  {
    id: "faq5",
    question: "Posso cancelar minha inscrição?",
    answer: "Sim, você pode cancelar sua inscrição até 7 dias antes do início do evento. Para isso, entre em contato através do email contato@flashclip.tech informando seu nome completo e curso escolhido."
  },
  {
    id: "faq6",
    question: "Os cursos são pagos?",
    answer: "Não, todos os cursos do FlashClip são gratuitos, destinados a estudantes da instituição e comunidade externa interessada em aprender programação."
  },
  {
    id: "faq7",
    question: "Qual o nível de conhecimento necessário para participar?",
    answer: "Cada curso tem pré-requisitos específicos, indicados na descrição. Em geral, os cursos de C#, TypeScript, Go e Ruby requerem conhecimentos básicos de programação, enquanto Prolog é recomendado para pessoas com alguma experiência prévia."
  },
  {
    id: "faq8",
    question: "Haverá material de apoio?",
    answer: "Sim, todos os participantes terão acesso a material digital de apoio, incluindo slides, exemplos de código e exercícios práticos através da plataforma do evento."
  }
];
