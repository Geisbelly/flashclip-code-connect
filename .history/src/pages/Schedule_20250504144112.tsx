...
const Schedule = () => {
  ...
  const schedulesByDate = scheduleData.reduce((acc: Record<string, any[]>, schedule) => {
    ...
    const date = new Date(Number(year), Number(month) - 1, Number(day)+1).toISOString().split('T')[0];
    if (!acc[date]) {
      acc[date] = [];
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 gap-8 mt-10">
              <div>
                ...
                <div className="prose prose-invert text-white/80">
                  <p>Os cursos presenciais serão realizados no Complexo Laboratorial de Informatica da Universidade Ulbra Palmas.</p>
                  ...
                  <p className="mb-2">O curso acontecerá no Complexo Laboratorial de Informatica, também conhecido como Engenharia de Software - Ulbra Palmas</p>
                  <iframe 
